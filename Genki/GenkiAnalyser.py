        	    			...
        	    			if link_url:
        	        			...
        	        			try:
        	        				...
        	        				if(r[link_url]['malicious']):
        	        					...
        	        					pdf.multi_cell(200, 10, txt=str(link_url), align="L")
        	        					pdf.set_font("Arial",size=10)
        	        					pdf.multi_cell(200, 10, txt="Malicieux:"+str(r[link_url]['malicious']), align="L")
        	        					pdf.multi_cell(200, 10, txt="plateforme:"+str(r[link_url]['platforms']), align="L")
        	        					pdf.multi_cell(200, 10, txt="type:"+str(r[link_url]['threats']), align="L")
        	        					pdf.multi_cell(200, 10, txt="cache:"+str(r[link_url]['cache']), align="L")
...
def check_for_javascript():
    try:
    	...
    	pdf.multi_cell(100, 10, txt="Une erreur est survenu pendant l'analyse des headers",align="L")
...
def analyze_embedded_objects():
	try:
   	  ...
   	  pdf.cell(200, 10, txt="Une erreur est survenu pendant l'analyse", ln=1, align="L")
	...
	try:
		...
		while(c!=1):
			if(response.json()['data']['attributes']['status']=='queued'):
				...
				pdf.set_font("Arial",size=10)
				pdf.cell(200, 10, txt="Inoffensif:"+str(response.json()['data']['attributes']['stats']['harmless']), ln=1, align="L")
				pdf.cell(200, 10, txt="Type de fichier non supportés:"+str(response.json()['data']['attributes']['stats']['type-unsupported']), ln=1, align="L")
				pdf.cell(200, 10, txt="Suspect:"+str(response.json()['data']['attributes']['stats']['suspicious']), ln=1, align="L")
				pdf.cell(200, 10, txt="Timeout confirmé:"+str(response.json()['data']['attributes']['stats']['confirmed-timeout']), ln=1, align="L")
				pdf.cell(200, 10, txt="Timeout:"+str(response.json()['data']['attributes']['stats']['timeout']), ln=1, align="L")
				pdf.cell(200, 10, txt="Malicieux:"+str(response.json()['data']['attributes']['stats']['malicious']), ln=1, align="L")
				pdf.cell(200, 10, txt="Non detecté:"+str(response.json()['data']['attributes']['stats']['undetected']), ln=1, align="L")
				pdf.cell(200, 10, txt="echecs:"+str(response.json()['data']['attributes']['stats']['failure']), ln=1, align="L")				
				pdf.set_font("Arial",'U',size=12)
				pdf.cell(200, 10, txt="Détails", ln=1, align="L")
				pdf.set_font("Arial",size=10)
				pdf.multi_cell(200, 10, txt=str(response.text), align="L")
				print(response.text)
	except(Exception):
		pdf.cell(200, 10, txt="Une erreur est survenu pendant l'analyse Virus Total", ln=1, align="L")
